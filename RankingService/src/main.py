...
async def lifespan(app: FastAPI):
    ...
    apply_overrides(settings, await load_overrides(pool, "ranking"))
    app.state.pool          = pool
    app.state.score_repo    = ScoreRepository(pool)
    app.state.score_service = ScoreService(pool)
    logger.info("RankingService ready")
    yield
    await pool.close()
    logger.info("RankingService shut down")


app = FastAPI(
    title="RankingService",
    description="Ranks NSE symbols by composite score and manages stage-based watchlists",
    version="0.1.0",
    lifespan=lifespan,
)
