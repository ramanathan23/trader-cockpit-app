"""
Dhan Option Chain client — on-demand REST API caller.

Rate limit: 1 unique request per 3 seconds (unlimited daily).
We enforce this via asyncio.Lock + minimum interval tracking.

Called on user interaction (button click), NOT automatically.
"""
...
class OptionChainClient:
    """Thread-safe Dhan option chain REST client with rate limiting."""
    ...
    async def get_expiry_list(
        ...
        data = await self._rate_limited_post(_EXPIRY_LIST_URL, payload)
    ...
    async def get_option_chain(
        ...
        Returns {
          ...
          "oc": {
            "strike": {
              "ce": { greeks, iv, oi, volume, bid/ask, ... },
              "pe": { greeks, iv, oi, volume, bid/ask, ... }
            }, ...
          }
        }
        """
        ...
        data = await self._rate_limited_post(_OPTION_CHAIN_URL, payload)
        return data.get("data", {})
