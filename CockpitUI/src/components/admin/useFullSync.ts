...
export function useFullSync() {
  ...
  async function runPipeline() {
    ...
    // indicators → scores run sequentially via SSE
    for (const step of PIPELINE_STEPS.filter(s => s.key === 'indicators' || s.key === 'scores')) {
    ...
    try {
      ...
      if (names.length === 0) {
        ...
        await Promise.allSettled(names.map(name => fetch(`/modeling/models/${name}/score-all`, { method: 'POST' })));
