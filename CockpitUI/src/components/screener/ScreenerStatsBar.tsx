    ...
    <div className="shrink-0 border-b border-subtle bg-base/70 px-4 py-2">
      <div className="flex flex-wrap gap-2">
        <div className="min-w-28 rounded-md border border-border bg-card px-3 py-2">
          <div className="text-[9px] font-bold tracking-[0.14em] uppercase text-muted">Universe</div>
          <div className="mt-1 text-[15px] font-black tabular-nums text-fg">{total}</div>
        </div>
        {stats.map(stat => (
          <div key={stat.key} className="min-w-36 rounded-md border border-border bg-card px-3 py-2">
            <div className="text-[9px] font-bold tracking-[0.14em] uppercase text-muted">{stat.label}</div>
            <div className="mt-1 flex items-baseline justify-between gap-3">
              <span className="text-[15px] font-black tabular-nums text-fg">
                {stat.count}<span className="text-[11px] text-muted">/{stat.eligible}</span>
              </span>
              <span className="text-[11px] font-bold tabular-nums" style={{ color: statColor(stat.pct) }}>
                {stat.pct.toFixed(0)}%
              </span>
            </div>
          </div>
